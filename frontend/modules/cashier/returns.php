...
// Include database and business logic
require_once '../../../backend/api/cashier/return_logic.php';

/* Data Retrieval: Fetching user and branch details for the sidebar */
...
/* Post Handling: Logic for Create, Update, and Delete actions */
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $action = $_POST['action'] ?? '';
    if ($action === 'create') {
        $res = createReturn(
            $_POST['sale_id'], $_POST['product_id'], (int)$_POST['quantity'],
            $_POST['reason'], $_POST['status'], $branch_id
        );
        $message = $res['message'];
...
// Refresh statistics and search data
    ...
    <div class="main">
        <div class="container">
            <div class="returns-card">
                ...
                <p style="color: #64748B; margin-bottom: 20px;">Branch: <?php echo htmlspecialchars($display_branch); ?></p>
                ...
                <div style="overflow-x: auto;">
                    <table class="returns-table">
                        <thead>
                            <tr>
                                <th>Return ID</th><th>Sale ID</th><th>Product ID</th><th>Product Name</th>
                                <th>Qty</th><th>Reason</th><th>Status</th><th>Date</th><th>Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php if (empty($returns_data)): ?>
                                <tr><td colspan="9" style="text-align:center;">No returns found.</td></tr>
                            <?php else: ?>
